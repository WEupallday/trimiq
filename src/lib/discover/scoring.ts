...
function rawFeatures(p: RawProduct): Raw {
  ...
  // the recent 3 days so a fresh spike can't inflate (and hide within) its own variance.
  const history = units.slice(0, -3);
  let ewma = 0, ewvar = 0; const alpha = 0.3; let init = false;
  ...
  const last3 = avg(units.slice(-3).map((u) => u.units));
  const g7 = win[7].growthRate;
  const isBreakout = units.length >= 8 && last3 > ewma + 2 * sigma && g7 > 0.5;

  let state = "stable";
  if (isBreakout) state = "breakout";
  else if (g7 < -0.3) state = "cooling";
  else if (Math.abs(g7) < 0.15) state = "stable";
...
export function scoreProducts(products: RawProduct[]): Scored[] {
  ...
  for (const cat of Object.keys(groups)) {
    ...
    const trendFor = (W: number, f: Raw) => {
      ...
      const raw =
        0.25 * zVel(log1p(f.win[W].velRecent)) +
        0.35 * zGrow(f.win[W].growthRate) +
        0.20 * zSpread(f.creatorSpread) -
        0.30 * (1 - f.confidence);
    ...
    const momentumFor = (W: number, f: Raw) => {
      const zVel = zscorer(group.map((x) => log1p(x.win[W].velRecent)));
      const zGrow = zscorer(group.map((x) => x.win[W].growthRate));
      return Math.round(100 * sigmoid(0.6 * zGrow(f.win[W].growthRate) + 0.4 * zVel(log1p(f.win[W].velRecent))));
    };
